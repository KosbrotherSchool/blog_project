class Api::MovieController < ApplicationController
  ...
  def rank_movies
    ...
    if params[:page] == nil
      ...
      if rank_type == 1
        movies = MovieRank.select("movies.id, title, small_pic, point").joins(:movie).where("rank_type = #{rank_type} and yahoo_link is not NULL and is_show = true").paginate(:page => params[:page], :per_page => 10)
        render :json => movies
      end
    end
      ...
      render :json => movie
        ...
        if movie_round <= 2
          ...
          movies = Movie.select('id, title, movie_class, small_pic').where("movie_round = #{movie_round} and yahoo_link is not NULL and is_this_week_new = false").order('publish_date_date ASC').paginate(:page => params[:page], :per_page => 10)
           ...
           movies = Movie.select('id, title, publish_date, movie_class, actors, movie_type, small_pic, title_eng').where("is_this_week_new = true and yahoo_link is not NULL")
        end
        render :json => movies   
    end
